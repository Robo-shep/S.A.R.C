...
class RocketLeagueEnv:
    ...
    def reset(self, stage):
        self.stage = stage; self.episode_steps = 0
        self.wall_contact_steps = 0  # <--- NEW: Initialize wall counter
        
        self.ball.vel[:] = 0; self.car1.vel[:] = 0; self.car2.vel[:] = 0
        self.car1.boost = 100; self.car2.boost = 100
        
        # (Existing stage setup logic remains the same...)
        if self.stage == 0:
            self.ball.pos = np.array([np.random.uniform(OFFSET_X+100, OFFSET_X+ARENA_W-100), np.random.uniform(OFFSET_Y+100, OFFSET_Y+ARENA_H-100)])
            angle = np.random.uniform(0, 6.28); dist = np.random.uniform(300, 500)
            self.car1.pos = self.ball.pos + np.array([np.cos(angle), np.sin(angle)]) * dist
            diff = self.ball.pos - self.car1.pos; self.car1.angle = np.degrees(np.arctan2(diff[1], diff[0])) + np.random.uniform(-10, 10)
            self.car2.pos = np.array([-1000.0, -1000.0])

        elif self.stage == 1:
            self.ball.pos = np.array([np.random.uniform(OFFSET_X+50, OFFSET_X+ARENA_W-50), np.random.uniform(OFFSET_Y+50, OFFSET_Y+ARENA_H-50)])
            self.car1.pos = np.array([np.random.uniform(OFFSET_X+50, OFFSET_X+ARENA_W-50), np.random.uniform(OFFSET_Y+50, OFFSET_Y+ARENA_H-50)])
    ...
    def step(self, action1, action2):
        ...
        for _ in range(PHYSICS_SUBSTEPS):
            ...
            # Check Wall Contact (Simple bounds check for Car 1)
        ...
        if touching_wall:
            ...
            self.wall_contact_steps = 0 # Reset if they leave the wall (optional, or keep cumulative)

        done = False; r1, r2 = 0.0, 0.0
        info = {'hit_ball': hit_ball, 'goal': False}
        
        # --- Goal Rewards (Increased as requested) ---
        if self.ball.pos[0] > OFFSET_X + ARENA_W and abs(self.ball.pos[1] - SCREEN_H/2) < 100:
            r1 += 30.0; r2 -= 30.0; done = True; info['goal'] = True # Increased from 10 to 30
        elif self.ball.pos[0] < OFFSET_X and abs(self.ball.pos[1] - SCREEN_H/2) < 100:
            r1 -= 30.0; r2 += 30.0; done = True # Increased penalty

        # --- Distance Reward (Small shaping) ---
        d1 = np.linalg.norm(self.car1.pos - self.ball.pos)
        r1 -= 0.001 
        if d1 < 500: r1 += (500 - d1) * 0.0001
        
        # --- Velocity Towards Ball (Reduced as requested) ---
        vec_to_ball = (self.ball.pos - self.car1.pos) / (d1 + 1e-5)
        vel_to_ball = np.dot(self.car1.vel, vec_to_ball)
        if vel_to_ball > 2.0: r1 += vel_to_ball * 0.001 # Reduced from 0.005

        # --- Boost Conservation Reward ---
        if self.car1.boost > 30.0:
            r1 += 0.005 # Small continuous reward for maintaining boost

        # --- Wall Punishment (> 30 timesteps) ---
        if self.wall_contact_steps > 30:
            r1 -= 0.05 # Penalty per step after staying on wall too long

        # --- Ball Movement Penalties ---
        # Punish if ball is moving towards own goal (Left side)
        ...
        if ball_vel_to_own_goal > 0:
            r1 -= ball_vel_to_own_goal * 0.05 # Punish speed towards own goal

        # --- Enemy Touch Punishment ---
        if enemy_hit_ball:
            r1 -= 1.0 # Punish immediately if enemy touches ball

        # --- Hit Rewards & Saves ---
        if hit_ball:
            r1 += 0.5
            
            # Harder Hit Reward (Increased multiplier)
            ...
            if shoot_vel > 0: 
                r1 += shoot_vel * 1.0 # Increased from 0.2 to 1.0
            
            # Save Reward
            # If ball was close to own goal (Left) and we hit it
            dist_to_own_goal = np.linalg.norm(self.ball.pos - np.array([OFFSET_X, SCREEN_H/2]))
            if dist_to_own_goal < 300:
                r1 += 5.0 # Bonus for "Saving" (hitting ball when it's near own net)

        # --- Step Limits ---
        max_steps = 1500 # Increased default steps per round (See Answer Below)
        if self.episode_steps > max_steps: done = True
...
if __name__ == "__main__":
    if not os.path.exists("saved_agents"): os.makedirs("saved_agents")
    
    env = RocketLeagueEnv()
    main_agent = PPO(15, 4, ACTION_STD_INIT)
    opponent_policy = ActorCritic(15, 4, ACTION_STD_INIT)
    
    if os.path.exists("curriculum_brain.pth"):
        print(">>> Loading Saved Brain...")
        try:
            main_agent.policy.load_state_dict(torch.load("curriculum_brain.pth"))
    ...
    memory = Memory()
    step, curr_stage = 0, START_STAGE
    stage0_hits, stage1_goals = 0, 0
    opponent_mode = "SELF" 

    obs1, obs2 = env.reset(curr_stage)
    
    try:
        while step < MAX_TIMESTEPS:
            step += 1
            
            # 1. Opponent Selection Logic (Check at Start of Episode)
            if env.episode_steps == 0: 
                rand = np.random.random()
                if curr_stage < 2: opponent_mode = "DUMB"
                elif rand < 0.2: opponent_mode = "DUMB"    
                elif rand < 0.4: opponent_mode = "HISTORY" 
                else: opponent_mode = "SELF"               
                ...
                if opponent_mode == "HISTORY":
                    ...
                    if len(files) > 0:
                        ...
                        try:
                            opponent_policy.load_state_dict(torch.load(chosen))
                        except:
                            opponent_mode = "SELF"
                    else: opponent_mode = "SELF"
            ...
            # 2. Get Actions
            a1 = main_agent.select_action(obs1, memory)
            
            if opponent_mode == "DUMB":
                a2 = np.array([0.0, 0.0, 0.0, 1.0], dtype=np.float32)
            else:
                state_opp = torch.FloatTensor(obs2.reshape(1, -1))
                a2, _ = opponent_policy.act(state_opp)
                a2 = a2.detach().numpy().flatten()

            # 3. Step
            (next_obs1, next_obs2), (r1, r2), done, info = env.step(a1, a2)
            memory.rewards.append(r1); memory.is_terminals.append(done)
            obs1, obs2 = next_obs1, next_obs2

            # 4. Logging & Curriculum
            if step % 1000 == 0:
                print(f"Step {step} | Stage {curr_stage} | Opponent: {opponent_mode} | R1: {r1:.3f}")

            if curr_stage == 0 and info['hit_ball']:
                stage0_hits += 1
                if stage0_hits % 5 == 0: print(f"Hit Ball: {stage0_hits}/1000")
                if stage0_hits >= 1000:
                    curr_stage = 1; print(">>> PROMOTED TO STAGE 1!"); done = True
            
            elif curr_stage == 1 and done:
                if info['goal']:
                    stage1_goals += 1
            ...
            if done: obs1, obs2 = env.reset(curr_stage)
